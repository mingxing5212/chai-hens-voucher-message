...
package me.mingxing5212.chaihens.voucher.message.wechat.service.cs;
...
public class CustomerServicesManager {
	...
	public void kfSessionCreate(String openId,String kfAccount,String text) throws WeChatException{
		...
		String resultStr = HttpClientUtils.post(CUSTOMSERVICE_KFSESSION_CREATE_POST_URL+this.accessToken,jsonObject.toJSONString());
	...
	public void kfSessionClose(String openId,String kfAccount,String text) throws WeChatException{
		...
		String resultStr = HttpClientUtils.post(CUSTOMSERVICE_KFSESSION_CLOSE_POST_URL + this.accessToken, jsonObject.toJSONString());
		WeChatUtil.isSuccess(resultStr);
	}
	/**
	 * 获取客户的会话状态 
	...
	public CustomerServicesSession getSession(String openId) {
		String resultStr = HttpClientUtils.get(CUSTOMSERVICE_KFSESSION_GETSESSION_GET_URL+this.accessToken+"&openid="+openId);
		try {
			WeChatUtil.isSuccess(resultStr);
	...
	public List<CustomerServicesSession> getSessionList(String kfAccount) {
		String resultStr = HttpClientUtils.get(CUSTOMSERVICE_KFSESSION_GETSESSIONLIST_GET_URL+this.accessToken+"&kf_account="+kfAccount);
		try {
			WeChatUtil.isSuccess(resultStr);
		} catch (WeChatException e) {
			return null;
	...
	public List<CustomerServicesSession> getWaitCaseList() {
		String resultStr = HttpClientUtils.get(CUSTOMSERVICE_KFSESSION_GETWAITCASE_GET_URL+this.accessToken);
		try {
			WeChatUtil.isSuccess(resultStr);
		} catch (WeChatException e) {
			return null;
	...
	public List<CustomerServices> getKfList() {
		String resultStr = HttpClientUtils.get(CUSTOMSERVICE_GETKFLIST_GET_URL+this.accessToken);
		try {
			WeChatUtil.isSuccess(resultStr);
		} catch (WeChatException e) {
			return null;
	...
	public List<CustomerServices> getOnlieKfList() {
		String resultStr = HttpClientUtils.get(CUSTOMSERVICE_GETONLIEKFLIST_GET_URL+this.accessToken);
		try {
			WeChatUtil.isSuccess(resultStr);
		} catch (WeChatException e) {
			return null;
	...
	public void kfAddAccount(String kfAccount,String nickName,String password) throws WeChatException{
		...
		String resultStr = HttpClientUtils.post(CUSTOMSERVICE_KFACCOUNT_ADD_POST_URL+this.accessToken,jsonObject.toJSONString());
		WeChatUtil.isSuccess(resultStr);
	}
	/**
	 * 设置客服信息
	...
	public void kfUpdateAccount(String kfAccount,String nickName,String password) throws WeChatException {
		...
		jsonObject.put("nickname", nickName);
		jsonObject.put("password", password);
		String resultStr =HttpClientUtils.post(CUSTOMSERVICE_KFACCOUNT_UPDATE_POST_URL+this.accessToken, jsonObject.toJSONString());
		WeChatUtil.isSuccess(resultStr);
	}
	/**
	 * 上传客服头像
	...
	public void kfUploadHeadImg(String kfAccount,File file) throws WeChatException{
		String resultStr = HttpClientUtils.postFile(CUSTOMSERVICE_KFACCOUNT_UPLOADHEADIMG_POST_URL+this.accessToken+"&kf_account="+kfAccount,PARAM_FILE, file);
		WeChatUtil.isSuccess(resultStr);
	}
	...
	public void kfDelAccount(String kfAccount) throws WeChatException{
		String resultStr = HttpClientUtils.post(CUSTOMSERVICE_KFACCOUNT_DEL_POST_URL+this.accessToken+"&kf_account="+kfAccount, "");
		WeChatUtil.isSuccess(resultStr);
	}
	/**
	 * 获取客服聊天记录
	...
	public List<Record> getRecord(long starttime,long endtime, int pageindex,int pagesize) {
		...
		String resultStr = HttpClientUtils.post(CUSTOMSERVICE_MSGRECORD_GETRECORD_POST_URL+this.accessToken, data.toJSONString());
		try {
			WeChatUtil.isSuccess(resultStr);
		} catch (WeChatException e) {
			return null;
